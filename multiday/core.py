    ...
    def concatenate_params(self, cell_indices: np.ndarray) -> None:
        ...
        for i in range(session_num):
            if self.cell_indices[i] != 0:
                ...
                behav_times_all = np.concatenate([behav_times_all, cp.deepcopy(self._res['correct_time'][i])+base_time])
                ...
                ms_time_behav_all = np.concatenate([ms_time_behav_all, self._res['ms_time_behav'][i] + base_time])
                ...
                ms_time_original = np.concatenate([ms_time_original, self._res['ms_time'][i] + base_time])
                place_fields.append(self._res['place_field_all'][i][int(self.cell_indices[i])-1])
            else:
            ...
            base_time = base_time + self._res['correct_time'][i][-1] + self.interv_time
            
        self.t_max = base_time/1000
