...
def indept_field_evolution_CI(
    ...
    P1 = np.histogram(real_distribution, bins=n_bin, range=_range, density=False)[0] / real_distribution.shape[0]
    print(np.histogram(real_distribution, bins=n_bin, range=_range, density=False)[0])
    
    print(np.sum(P), np.sum(P1))
